...
type QualitySummary = { verifiedFacts: number; evidenceOnly: number; exceptions: number; verifiedCodes: string[]; missingCodes: string[]; missingRequiredCodes: string[]; readyToCommit: boolean };
...
export function PdfExtractionPanel() {
  ...
  async function review(candidate: Candidate, decision: "PENDING" | "ACCEPTED" | "REJECTED", canonicalAccountId?: string | null) {
    ...
    try {
      ...
      setSelected((current) => current ? { ...current, candidates: current.candidates.map((item) => item.id === candidate.id ? { ...item, status: decision, canonicalAccountId: targetAccountId ?? null, canonicalAccount: mappedAccount ?? null } : item) } : current);
      await openRun(selected.id, false);
      refreshPipeline();
  ...
  const missingRequiredCodes = quality?.missingRequiredCodes ?? [];
  const optionalMissingCodes = quality?.missingCodes.filter((code) => !missingRequiredCodes.includes(code)) ?? [];
  const qualityReady = quality?.readyToCommit ?? false;

  async function commitRun() {
    if (!selected || committed) return;
    if (!qualityReady) return setMessage(`Save masih dikunci. Critical facts wajib belum lengkap: ${missingRequiredCodes.join(", ") || "periksa exception quality"}.`);
  ...
  return <>
    ...
    {selected && <section className="card" id="report-review-workspace">
      ...
      {quality && <div className="card" style={{ marginTop: 14 }}><h3>Canonical QC Summary</h3><div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 10 }}><PipelineStatusBadge status={`GREEN FACTS ${quality.verifiedFacts}`} /><PipelineStatusBadge status={`EVIDENCE ONLY ${quality.evidenceOnly}`} /><PipelineStatusBadge status={`EXCEPTIONS ${quality.exceptions}`} /></div><p className="form-hint" style={{ marginTop: 10 }}>Verified: {quality.verifiedCodes.join(", ") || "—"}</p>{missingRequiredCodes.length > 0 && <p className="form-hint">Critical facts wajib belum lengkap: {missingRequiredCodes.join(", ")}</p>}{optionalMissingCodes.length > 0 && <p className="form-hint">Tidak dilaporkan secara aman (tidak dibuat-buat): {optionalMissingCodes.join(", ")}</p>}</div>}
      ...
      <div className="card" style={{ marginTop: 16 }}><h3>Review Summary</h3><div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 10 }}>{committed ? <PipelineStatusBadge status={`COMMITTED ${committedCandidates}`} /> : <PipelineStatusBadge status={`GREEN ${accepted}`} />}<PipelineStatusBadge status={`EVIDENCE ${rejected}`} /><PipelineStatusBadge status={`EXCEPTIONS ${pending}`} /></div>{!committed && pending > 0 && <div className="callout" style={{ marginTop: 12 }}>Save dikunci karena masih ada exception quality yang harus diselesaikan.</div>}{!committed && pending === 0 && missingRequiredCodes.length > 0 && <div className="callout" style={{ marginTop: 12 }}><b>SAVE LOCKED.</b> Critical facts wajib belum lengkap: {missingRequiredCodes.join(", ")}.</div>}{!committed && qualityReady && <div className="callout" style={{ marginTop: 12 }}><b>QC PASSED.</b> Bro cukup tekan Save; hanya canonical GREEN yang masuk PostgreSQL.</div>}{committed && <div className="callout" style={{ marginTop: 12 }}><b>DATABASE RESULT:</b> {committedCandidates} canonical facts masuk PostgreSQL; {rejected} evidence-only tidak dimasukkan.</div>}<div style={{ height: 12 }} /><button className="btn" type="button" onClick={commitRun} disabled={loading || !qualityReady || accepted === 0 || committed}>{committed ? "SAVED TO POSTGRESQL 🔒" : pending > 0 ? `SAVE locked — ${pending} exceptions` : missingRequiredCodes.length > 0 ? `SAVE locked — ${missingRequiredCodes.length} critical facts missing` : "SAVE VERIFIED CANONICALS TO POSTGRESQL"}</button></div>
    </section>}
  </>;
}
