...
type PipelineSummary = { all: SummaryCount; needReview: SummaryCount; readyToCommit: SummaryCount; committed: SummaryCount; failed: SummaryCount };
...
export function FinancialReportPipeline({ onOpen, refreshKey = "" }: Props) {
  ...
  useEffect(() => {
    ...
    async function load(showSpinner = true) {
      ...
      try {
        ...
        if (nextRuns.some((run: PipelineRun) => run.kind === "JOB" && (run.status === "UPLOADED" || run.status === "SUBMITTING" || run.status === "PROCESSING"))) {
  ...
  return <section style={{ marginTop: 20 }}>
    ...
    <div style={{ display: "grid", gap: 10, marginTop: 12 }}>{runs.map((run) => {
      ...
      const processing = run.kind === "JOB" && (run.status === "UPLOADED" || run.status === "SUBMITTING" || run.status === "PROCESSING");
