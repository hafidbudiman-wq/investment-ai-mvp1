...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (existing) return NextResponse.json({ ok: true, duplicate: true, run: existing, message: "PDF yang sama sudah pernah diproses. Tidak dibuat duplikat." });

    const year = /^\d{4}$/.test(yearValue) ? Number(yearValue) : null;
    const allowedPeriods = ["Q1", "H1", "Q3", "FY", "MONTHLY"] as const;
    const safePeriod = allowedPeriods.includes(periodType as any) ? (periodType as (typeof allowedPeriods)[number]) : null;

    const run = await prisma.extractionRun.create({
      data: { companyId, fileName: file.name, mimeType: file.type, fileSize: file.size, checksum, year, periodType: safePeriod, currency: company.currency, status: "PROCESSING" },
    });
    runId = run.id;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const candidate of extracted.candidates) {
        ...
        await tx.extractionCandidate.create({
          data: {
            ...
            mappingMethod: canonical ? "AI_SUGGESTED" : null,
            status: "PENDING",
          },
        });
      }
      ...
      await tx.extractionRun.update({
        ...
        data: {
          ...
          year: extracted.detectedYear || year,
          periodType: extracted.detectedPeriodType || safePeriod,
    ...
    return NextResponse.json({
      ...
      message: `PDF selesai diekstrak: ${extracted.candidates.length} kandidat menunggu review. Belum ada nilai yang ditulis ke canonical database.`,
    });
  } catch (error) {
    console.error("pdf-extraction-upload-failed", error);
    if (runId) {
      await prisma.extractionRun.update({ where: { id: runId }, data: { status: "FAILED", errorMessage: error instanceof Error ? error.message.slice(0, 2000) : "Extraction failed" } }).catch(() => undefined);
    }
