...
export async function POST(request: Request) {
  ...
  try {
    ...
    const metadataMismatch = Boolean((year && extracted.detectedYear && extracted.detectedYear !== year) || (safePeriod && extracted.detectedPeriodType && extracted.detectedPeriodType !== safePeriod));
    const metadataMessage = metadataMismatch ? `AI detected ${extracted.detectedPeriodType ?? "?"} ${extracted.detectedYear ?? "?"}, but upload selection ${safePeriod ?? "?"} ${year ?? "?"} remains authoritative.` : null;
    await prisma.$transaction(async (tx) => {
      ...
      await tx.extractionRun.update({ where: { id: run.id }, data: { status: "PENDING_REVIEW", pageCount: extracted.pageCount, year, periodType: safePeriod, currency: extracted.detectedCurrency || company.currency, unitScale: extracted.detectedUnitScale || null, errorMessage: metadataMessage } });
    });
    return NextResponse.json({ ok: true, duplicate: false, runId: run.id, candidateCount: extracted.candidates.length, chunkCount: chunks.length, processingMode: preflight.processingMode, preflightReason: preflight.reason, metadataMismatch, metadataMessage, message: `PDF selesai diproses via ${preflight.processingMode}: ${extracted.candidates.length} kandidat menunggu review. Belum ada nilai yang ditulis ke canonical database.` });
