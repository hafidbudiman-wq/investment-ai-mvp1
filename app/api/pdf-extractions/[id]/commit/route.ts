...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const run = await prisma.extractionRun.findUnique({
      where: { id },
      include: {
        candidates: { include: { canonicalAccount: true } },
        company: true,
      },
    });
    ...
    const duplicateCodes = accepted.map((c) => c.canonicalAccountId!).filter((id, index, arr) => arr.indexOf(id) !== index);
    ...
    if (existingReport && existingReport.id !== run.reportId) {
      return NextResponse.json({ error: `${run.company.ticker} ${run.periodType} ${run.year} sudah ada di canonical database. Commit diblokir agar data lama tidak tertimpa.` }, { status: 409 });
    }
    ...
    const reportId = await prisma.$transaction(async (tx) => {
      const report = existingReport ?? await tx.financialReport.create({
        data: {
          ...
          periodEnd: periodEnd(run.year!, run.periodType!),
          sourceType: "PDF",
          sourceFile: run.fileName,
          status: "REVIEWED",
          createdBy: "pdf-ai-extraction",
        },
      });
      ...
      for (const type of statementTypes) {
        const statement = await tx.financialStatement.upsert({
          where: { reportId_statementType: { reportId: report.id, statementType: type } },
          update: { currency: run.currency || run.company.currency, unitScale: run.unitScale || 1 },
          create: { reportId: report.id, statementType: type, currency: run.currency || run.company.currency, unitScale: run.unitScale || 1 },
        });
      ...
      for (const candidate of accepted) {
        ...
        await tx.financialEntry.create({
          data: {
            ...
            rawText: candidate.sourceText || candidate.rawValue,
            value: candidate.numericValue!,
            scale: candidate.scale,
            currency: candidate.currency || run.currency || run.company.currency,
            isEstimated: true,
            isVerified: true,
            confidence: Math.min(candidate.extractionConfidence ?? 0, candidate.mappingConfidence ?? 0),
      ...
      await tx.auditLog.create({ data: { reportId: report.id, action: "PDF_AI_COMMIT", actor: "web-user", entity: "ExtractionRun", entityId: run.id, note: `${accepted.length} reviewed PDF candidates committed to canonical database.` } });
