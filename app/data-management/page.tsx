...
export default async function DataPage() {
  ...
  return <>
    ...
    <section className="data-management-layout">
      <div className="card data-card"><div className="section-title"><h2>Account Mapping</h2></div><MappingManager initialMappings={fallbackMappings} /></div>
      <div className="card validation-card"><h2>Validation Status</h2>
        {reports.length === 0 && <p>Belum ada laporan keuangan tersimpan.</p>}
        <div className="validation-list">{reports.map((report) => <div className="validation-item" key={report.id}><b>{report.company.ticker} · {report.periodType} {report.year}</b><span className={report.status === "VERIFIED" ? "positive" : "warning"}>{report.status === "VERIFIED" ? "✓ Verified" : "⚠ Review"}</span>{report.warnings.map((warning) => <small className="danger" key={warning.id}>⚠ {warning.message}</small>)}</div>)}</div>
      </div>
    </section>
  </>;
}
