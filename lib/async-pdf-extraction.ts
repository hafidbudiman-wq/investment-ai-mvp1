...
export async function createAsyncJob(input: { id: string; fileName: string; mimeType: string; fileSize: number; checksum: string; preflight: unknown; bytes: Buffer }) {
  ...
  await prisma.$executeRawUnsafe(
    ...
    input.id, input.fileName, input.mimeType, input.fileSize, input.checksum, JSON.stringify(input.preflight), input.bytes,
  );
}
...
export async function submitQueuedAsyncJob(id: string) {
  ...
  try {
    ...
    const background = await submitFinancialPdfBackground({ bytes: Buffer.from(job.fileData), fileName: job.fileName, knownCompanies: companies, accounts, preflight: job.preflight as Parameters<typeof submitFinancialPdfBackground>[0]["preflight"] });
...
export async function kickQueuedAsyncExtractionJobs(limit = 2) {
  await ensureAsyncExtractionTable();
  await prisma.$executeRawUnsafe(`UPDATE "AsyncExtractionJob" SET "status"='UPLOADED',"updatedAt"=CURRENT_TIMESTAMP WHERE "status"='SUBMITTING' AND "openAiResponseId" IS NULL AND "fileData" IS NOT NULL AND "updatedAt" < CURRENT_TIMESTAMP - INTERVAL '5 minutes'`);
  const queued = await prisma.$queryRawUnsafe<Array<{ id: string }>>(`SELECT "id" FROM "AsyncExtractionJob" WHERE "status"='UPLOADED' AND "openAiResponseId" IS NULL ORDER BY "createdAt" ASC LIMIT $1`, limit);
...
async function finalizeJob(job: AsyncJob) {
  ...
  const company = companies.find((item) => normalize(item.ticker) === detectedTicker) ?? companies.find((item) => normalize(item.name) === detectedName);

  await prisma.$executeRawUnsafe(`UPDATE "AsyncExtractionJob" SET "detectedTicker"=$2,"detectedCompanyName"=$3,"detectedYear"=$4,"detectedPeriodType"=$5,"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=$1`, job.id, extracted.detectedCompanyTicker, extracted.detectedCompanyName, extracted.detectedYear, extracted.detectedPeriodType);
  ...
  const runId = await prisma.$transaction(async (tx) => {
    const run = await tx.extractionRun.create({ data: { companyId: company.id, fileName: job.fileName, mimeType: job.mimeType, fileSize: job.fileSize, checksum: job.checksum, year: extracted.detectedYear, periodType: extracted.detectedPeriodType, currency: extracted.detectedCurrency || company.currency, unitScale: extracted.detectedUnitScale || null, pageCount: extracted.pageCount, status: "PROCESSING", parserVersion: "mvp-1.2d-v6-ack-first-background" } });
    const chunkIds = new Map<number, string>();
    for (let index = 0; index < chunks.length; index++) {
      const chunk = chunks[index];
      const stored = await tx.extractionChunk.create({ data: { runId: run.id, ordinal: index + 1, chunkType: chunk.chunkType, pageStart: chunk.pageStart, pageEnd: chunk.pageEnd, section: chunk.section, text: chunk.textSummary.slice(0, 12000), textHash: sha256(Buffer.from(chunk.textSummary)), metadata: { processingMode: preflight.processingMode, preflightConfidence: preflight.confidence, preflightReason: preflight.reason, openAiResponseId: job.openAiResponseId } } });
