...
function normalize(value: string): string { return value.normalize("NFKC").toLowerCase().replace(/[^a-z0-9\u00c0-\u024f]+/g, " ").replace(/\s+/g, " ").trim(); }
function countAnchor(text: string, anchor: string): number { let count = 0; let cursor = 0; while ((cursor = text.indexOf(anchor, cursor)) >= 0) { count += 1; cursor += anchor.length; } return count; }

function edgeRegions(page: P0AIndexedPage): string[] {
  if (!page.tokens.length) return [page.normalizedText.slice(0, 1_200)];
  const low = page.height * 0.15; const high = page.height * 0.85;
...
function noteMasthead(page: P0AIndexedPage): boolean {
  return edgeRegions(page).some((edge) => {
    const noteIdentity = (edge.includes("catatan") && edge.includes("laporan") && edge.includes("keuangan")) || (edge.includes("notes") && edge.includes("financial") && edge.includes("statements"));
...
function containsWords(text: string, words: readonly string[]): boolean { const tokens = new Set(text.split(" ").filter(Boolean)); return words.every((word) => tokens.has(word)); }
...
function epsNumericStructure(page: P0AIndexedPage): boolean {
  ...
  for (const match of page.text.matchAll(/\(?[-−–—]?\d[\d.,]*\)?/g)) {
    ...
    if (value > 0 && value < 1_000_000 && !looksLikeYear) hasPerShare = true;
...
function hasBasicEpsSemantics(text: string): boolean {
  return text.includes("basic earnings per share") || text.includes("basic diluted earnings per share") || text.includes("laba per saham dasar")
    || containsWords(text, ["basic", "earnings", "share"]) || containsWords(text, ["laba", "saham", "dasar"]);
}
function hasWeightedShareSemantics(text: string): boolean {
  return text.includes("weighted average number of shares") || text.includes("weighted average number of ordinary outstanding share") || text.includes("weighted average number of ordinary shares outstanding")
    || text.includes("jumlah rata rata tertimbang saham") || text.includes("rata rata tertimbang jumlah saham biasa yang beredar") || text.includes("rata rata tertimbang saham biasa yang beredar")
    || containsWords(text, ["weighted", "average", "ordinary", "share"]) || containsWords(text, ["rata", "tertimbang", "saham", "beredar"]);
}
function epsCalculationPage(page: P0AIndexedPage): boolean { return hasBasicEpsSemantics(page.normalizedText) && hasWeightedShareSemantics(page.normalizedText) && epsNumericStructure(page); }
...
function classify(page: P0AIndexedPage): Omit<P0ARoutedPage, keyof P0AIndexedPage> {
  ...
  if (noteMasthead(page)) return { pageClass: "TARGETED_NOTE", statementType: "NOTE", confidence: 0.995, matchedAnchors: ["notes masthead"] };
  ...
  if (epsCalculationPage(page)) return { pageClass: "TARGETED_NOTE", statementType: "NOTE", confidence: 0.98, matchedAnchors: ["eps calculation table"] };
...
function targetedScore(page: P0ARoutedPage, requirement: P0ARequirement): { anchorScore: number; score: number } {
  ...
  if (requirement.id === "WEIGHTED_AVG_SHARES_REPORTED") {
    const semanticTable = epsCalculationPage(page); const scoped = semanticTable && epsCalculationIsScoped(page);
