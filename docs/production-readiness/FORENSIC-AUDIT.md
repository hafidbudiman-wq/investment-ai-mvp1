...
| Critical (resolved) | Canonical investor set was absent | The approved 19-fact MVP1 core set is now centralized in `critical-accounts.config.ts` and covered by deterministic regression tests | Completeness can be calculated without asking the investor to map rows manually | Central config, API summary, and golden tests | Resolved in PR #22 follow-up |
